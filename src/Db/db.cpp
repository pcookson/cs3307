...
void Db::Connect() {
	...
	mysql_real_connect(conn_, MYSQL_SERVER, MYSQL_USERNAME, MYSQL_PASSWORD, MYSQL_DATABASE, MYSQL_REMOTE_PORT, NULL, 0);
}

string Db::GetElementByName(std::string const& name, vector<string> const& column_names, vector<string> const& row)
{
	...
	//find the index
	for (vector<string>::const_iterator it = column_names.begin(); it != column_names.end(); ++it, ++x) {
...
std::string Db::ParamertizedQuery(std::string query, vector<string> values)
{
	...
	for (string::iterator it = query.begin(); it != query.end(); ++it)
	{
		if (*it == '?')
		{
			...
			for(string::iterator val_it = val.begin(); val_it != val.end(); ++val_it)
			{
